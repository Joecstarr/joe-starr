import notewptt from "/note_wptt_wasm.mjs";
import { SVG, Point } from "https://cdnjs.cloudflare.com/ajax/libs/svg.js/3.2.5/svg.esm.js";
...
class BandDraw {
    ...
    __world_coor_bottom_right = (outerGroup, innerGroup) => {
        ...
        const pointInContainingGroup = new Point(transformedX, transformedY).transform(
            transformMatrix,
        );
    ...
    __connect_vh = (group, x1g1, y1g1, x2g1, y2g1, x1g2, y1g2, x2g2, y2g2) => {
        var top =
            ...
            y1g1 +
            " C " +
            (x1g1 - this.eccentricity * this.__gap) +
            "," +
            y1g1 +
            " " +
            x2g1 +
            "," +
            (y2g1 - this.eccentricity * this.__gap) +
            " " +
            x2g1 +
            "," +
            y2g1;
        var bottom =
            ...
            y1g2 +
            " C " +
            (x1g2 - this.eccentricity * this.__gap) +
            "," +
            y1g2 +
            " " +
            x2g2 +
            "," +
            (y2g2 + this.eccentricity * this.__gap) +
            " " +
            x2g2 +
            "," +
            y2g2;
    ...
    __connect_cl = (group, x1g1, y1g1, x2g1, y2g1, x1g2, y1g2, x2g2, y2g2) => {
        var top =
            ...
            x1g1 +
            "," +
            (y1g1 + this.eccentricity * this.__gap) +
            " " +
            (x2g1 + this.eccentricity * this.__gap) +
            "," +
            (y2g1 + this.eccentricity * this.__gap) +
            " " +
            x2g1 +
        ...
        var bottom =
            ...
            x1g2 +
            "," +
            (y1g2 - this.eccentricity * this.__gap) +
            " " +
            (x2g2 + this.eccentricity * this.__gap) +
            "," +
            (y2g2 - this.eccentricity * this.__gap) +
            " " +
            x2g2 +
    ...
    __proc_child = (child, group, depth) => {
        ...
        band.transform({
            translateX:
                this.__xoffset + band.bbox().height / 2 - band.bbox().width / 2 + this.__gap,
    ...
    draw = () => {
        ...
        if (0 < this.__band.children.size()) {
            if (
                ...
                band = new Integral(
                    this.__band.weights.get(this.__band.children.size()),
                    this.eccentricity,
                    this.crossing_color,
                    this.stroke_width,
                    group,
        ...
        if (1 < this.__wc.length) {
            ...
            var right_child_br = this.__world_coor_bottom_right(
                group,
                this.__wc[this.__wc.length - 1],
            );
